...
OUTPUT_DIR = joinpath(@__DIR__, "..", "figure_data", "CATKE_inference_results")

#####
##### Run CATKE model simulation
#####

"""
    run_inference_CATKE(data, grid, OUTPUT_DIR, prefix)

Run column model simulation using CATKE turbulence closure.
...
function run_inference_CATKE(data, grid, OUTPUT_DIR, prefix)
    ...
    # Build model with e as tracer for TKE
    ...
    # Diffusivities from CATKE closure
...
for (field_dataset, case) in zip(field_datasets, cases)
    run_inference_CATKE(field_dataset, grid, OUTPUT_DIR, case)
end
