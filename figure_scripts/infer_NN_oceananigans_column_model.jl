...
function run_inference_NN(data, grid, OUTPUT_DIR, prefix)
        ...
        # Access diffusivity fields from base closure (first in tuple)
        ...
        # Access residual fluxes from NN closure (second in tuple)
        ...
        # Total fluxes (base closure diffusive fluxes)
        uw = ν * ∂u∂z
        vw = ν * ∂v∂z
        wT = κ * ∂T∂z
        wS = κ * ∂S∂z
...
results = [run_inference_NN(field_dataset, grid, OUTPUT_DIR, case) for (field_dataset, case) in zip(field_datasets, cases)]