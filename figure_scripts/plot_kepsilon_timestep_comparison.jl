...
Δt_labels = [L"\Delta t = \text{1 minute}", L"\Delta t = \text{30 minutes}", L"\Delta t = \text{1 hour}", L"\Delta t = \text{2 hours}"]
...
#####
##### Load k-epsilon model results for different timesteps
#####
...
with_theme(theme_latexfonts()) do
    ...
    #####
    ##### Compute axis limits (exclude last timestep for u-velocity limit)
    #####
    
    ulim = (find_min([interior(ubar_data[n]) for ubar_data in ubar_datas[1:end-1]]...) - 1e-2, 
            find_max([interior(ubar_data[n]) for ubar_data in ubar_datas[1:end-1]]...) + 1e-1)
