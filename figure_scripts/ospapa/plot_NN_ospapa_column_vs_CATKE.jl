...
closure_strs = ["CATKE", "NORi", "NORiBase"]
...
with_theme(theme_latexfonts()) do
    ...
    for i in eachindex(subsample_range)
        ...
        lines!(axTs[i], vec(T_sim_means[1, 1, :, subsample_range[i]]), zCs, linewidth=profile_linewidth, color=colors[6], label="CATKE closure")
    ...
    display(fig)
    # save(joinpath(@__DIR__, "..", "..", "figures", "NN_ospapa_comparison_column_vs_CATKE.pdf"), fig)
end